...
const PEOPLES = require("./data/persons").peoples.map(person => {
...
exports.get = (req, res) => {
  ...
  if (!person) {
    return res
      .status(404)
      .json({ error: "La personne avec l'id \"" + id + "\" n'existe pas." });
  }
...
exports.create = (req, res) => {
  ...
  console.log(
    "Create person : lastname=" + lastname + ", firstname=" + firstname
  );

  var found = _.findWhere(PEOPLES, {
    lastname: lastname,
    firstname: firstname
  });
  if (found) {
    return res.status(409).json({
      error: 'La personne "' + lastname + " " + firstname + '" existe déjà.'
    });
  }
